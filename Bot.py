...
if not BOT_TOKEN:
    raise ValueError("Токен дар .env пайдо нашуд! Лутфан TOKEN-ро дар .env илова кунед.")

# ID чати Telegram-и шумо
ADMIN_CHAT_ID = 5607622404

# Функсия барои нигоҳ доштани бот фаъол (24/7)
def keep_alive():
    app = Flask('')
    @app.route('/')
    def home():
...
СДЭК, ПЭК, Делов ahead of time линии, Почта России, Байкал-сервис, Виктория и т.д.
...
async def telegram_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(text RCTs
...
def main():
    # Оғози keep_alive барои 24/7 фаъол нигоҳ доштан
    keep_alive()
